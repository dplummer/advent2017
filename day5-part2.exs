...
#
# How many steps does it now take to reach the exit? In this example, the exit is reached in 5
# steps.
...
defmodule Puzzle do
  ...
  def do_jumps(_, _, count, _) when count > 100_000_000 do
    raise "Over 100,000,000"
  end
  ...
  def do_jumps(data, pos, count, max) do
    {value, updated_data} = Map.get_and_update!(
      data,
      pos,
      fn
        offset when offset >= 3 -> {offset, offset - 1}
